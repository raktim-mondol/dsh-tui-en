...
```sh
# Install the CLI and this plugin globally (the plugin ships the dsh-tui-en command)
...
| `/exit` (alias `/quit` `/q`) | none | Exit dsh-tui-en |
...
- `/workspace`: `resume` / `rename <name>` / `open <path|file:// URI>`
  (opens and starts a new session); the `dsh-tui-en <path>` launcher also
...
- VS Code: use the companion extension `dsh-tui-vscode` (on the
  Marketplace, backed by a real integrated terminal), or run `dsh-tui-en`
