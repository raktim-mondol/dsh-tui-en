...

{
  ...
  assert(await settled(() => screenText(term, ROWS).includes('Start a new conversation') && !screenText(term, ROWS).includes('新开会话')), '/lang zh: built-in /new stays English')
  assert(await settled(() => screenText(term, ROWS).includes('Compact the conversation history') || screenText(term, ROWS).includes('Compact the conversation')), '/lang zh: compact stays English')
  assert(await settled(() => screenText(term, ROWS).includes('Toggle plan mode')), '/lang zh: external /plan stays English')
  assert(await settled(() => screenText(term, ROWS).includes('Registry fallback text')), '/lang zh: unlisted external falls back to registry text')
  assert(await settled(() => !screenText(term, ROWS).includes('新开会话')), '/lang zh: no leftover Chinese descriptions')
...

{
  ...
  assert(await settled(() => screenText(term, ROWS).includes('/new — Start a new conversation')), '/lang zh: help menu shows /new English')
  assert(await settled(() => screenText(term, ROWS).includes('/rewind — Rewind')), '/lang zh: help menu shows rewind English')
