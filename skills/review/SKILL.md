...
2. Read the code with these lenses:
   - **Design**: does the structure match the problem? Clear ownership, sane seams, no over-engineering?
   - **Correctness**: boundary conditions, error handling, concurrency, resource lifetime.
   - **Maintainability**: naming, duplication, dead code, complexity hotspots, comment quality.
   - **Tests**: are the important behaviors covered? Are tests asserting behavior rather than implementation?
